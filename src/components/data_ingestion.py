...
class DataIngestion:
    def __init__(self):
        self.data_ingestion_config = DataIngestionConfig() #calling the dataingestionconfig class
        
    def iniitiate_data_ingestion(self):
        try:
            df = pd.read_csv(DATASET_PATH)

            #df = pd.read_csv(os.path.join('D:\portfolio projects\Delivery-time-E2E-project\Data\finalTrain.csv'))
            
            #creating the raw data folder if not existed
